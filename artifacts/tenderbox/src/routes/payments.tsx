...
import {
  Clock, CheckCircle, AlertTriangle, Lock, Download,
  ToggleLeft, ToggleRight, Shield, Fingerprint, ChevronDown, ChevronUp,
} from "lucide-react";
...
const SIGNER_NAMES: Record<StepId, string> = {
  qs: "QS Eng. D. Maphumulo",
  pa: "Arch. S. Pillay",
  invoice: "Sizwe Construction",
  pm: "PM Mr. T. Mokoena",
  pmu: "PMU Director Ms. B. Khumalo",
  cfo: "CFO Mr. R. van Wyk",
  mm: "Municipal Manager Mr. K. Ndlovu",
  released: "Tenderbox Payment Engine",
};
    ...
    setTimeout(() => {
      setApprovals((prev) => {
        ...
        if (nextStep === "released") {
          ...
          setTimeout(() => toast("Payment released — R1,245,000 transferred to Sizwe Construction (Pty) Ltd", "success"), 200);
        } else {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-colors ${contractorView ? "bg-primary text-primary-foreground" : "border border-border bg-card text-foreground hover:bg-muted"}`}
          >
            ...
            <div>
              ...
              <div className="space-y-2">
                {STEP_DEFS.map((step) => {
                  ...
                  return (
                    <div key={step.id} className={`flex items-center gap-3 rounded-lg border p-3 transition-all ${
                      approval.completed ? "border-success/20 bg-success/5" :
                      isCurrent ? "border-primary/30 bg-primary/5" :
                      "border-border bg-muted/20"
                    }`}>
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
                        approval.completed ? "bg-success/20 text-success" :
                        isCurrent ? "bg-primary/20 text-primary" :
                        "bg-muted text-muted-foreground"
                      }`}>
                        {approval.completed ? <CheckCircle className="h-5 w-5" /> :
                         isCurrent ? <div className="h-3 w-3 rounded-full bg-primary animate-pulse" /> :
                         <div className="h-2 w-2 rounded-full bg-current" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`text-sm font-medium ${approval.completed ? "text-foreground" : isCurrent ? "text-primary" : "text-muted-foreground"}`}>
                            {step.label}
                          </span>
                          {step.window !== "—" && <span className="text-xs text-muted-foreground shrink-0">{step.window}</span>}
                        </div>
                        {approval.completed && approval.date && (
                          <div className="text-xs text-muted-foreground">{approval.signedBy ?? "Completed"} · {formatDateTime(approval.date)}</div>
                        )}
                        {isCurrent && <div className="text-xs text-primary">Awaiting signature · {step.window} window</div>}
                      </div>
                      {isSignable && (
                        <button
                          onClick={() => handleSign(stepId)}
                          disabled={signing === stepId}
                          className="shrink-0 inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-60"
                        >
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {PAYMENT_HISTORY.map((p, i) => (
                    <tr key={p.id} className={`${i % 2 === 1 ? "bg-muted/30" : "bg-card"} hover:bg-primary/5 cursor-pointer`} onClick={() => setExpandedHistory(expandedHistory === p.id ? null : p.id)}>
                      ...
                      <td className="px-6 py-3">
                        <span className={`font-medium ${p.daysTaken < 20 ? "text-success" : p.daysTaken <= 28 ? "text-warning" : "text-danger"}`}>{p.daysTaken} days</span>
                      </td>
                      <td className="px-6 py-3">
                      ...
                      <td className="px-6 py-3 text-right">
                        <button onClick={(e) => { e.stopPropagation(); toast(`Downloading receipt for ${p.id}`, "info"); }} className="text-xs text-primary hover:underline">Download</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
