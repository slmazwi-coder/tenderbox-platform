...
export const Route = createFileRoute("/contractor/profile")({ component: GuardedProfilePage });
...
function ExperienceTab() {
  ...
  const verCls = (v: string) => v === "Verified" ? "text-success" : v === "Pending" ? "text-warning" : "text-muted-foreground";
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-border bg-card shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {experience.map((e, i) => (
                <tr key={e.id} className={i % 2 === 1 ? "bg-muted/30" : "bg-card"}>
                  <td className="px-6 py-3 font-medium text-foreground max-w-[200px]">
                    <div className="line-clamp-2">{e.project}</div>
                    <div className="text-xs text-muted-foreground">{e.sector}</div>
                  </td>
                  <td className="px-6 py-3 text-muted-foreground">{e.client}</td>
                  <td className="px-6 py-3 text-muted-foreground">{e.role}</td>
                  <td className="px-6 py-3 font-medium text-foreground">{formatZAR(e.value)}</td>
                  <td className="px-6 py-3">
                    <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-xs font-medium ${e.completion === "Completed" ? "border-success/20 bg-success/10 text-success" : "border-primary/20 bg-primary/10 text-primary"}`}>
                      {e.completion === "Completed" ? <CheckCircle className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                      {e.completion}
                    </span>
                  </td>
                  <td className="px-6 py-3">
                    <span className={`text-xs font-medium ${verCls(e.verification)}`}>{e.verification}</span>
                  </td>
                  <td className="px-6 py-3 text-right">
                    <button onClick={() => setViewEntry(e)} className="inline-flex items-center gap-1 rounded-md border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted">
                      <Eye className="h-3.5 w-3.5" />View
                    </button>
                  </td>
                </tr>
        ...
        <Modal title="Add Project Experience" onClose={() => setShowAdd(false)} size="lg">
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {[
                { label: "Project Name *", key: "project", placeholder: "e.g. Upgrading of Access Roads Phase 3" },
                { label: "Client / Employer *", key: "client", placeholder: "e.g. OR Tambo District Municipality" },
                { label: "Contract Value (ZAR) *", key: "value", placeholder: "e.g. 8500000" },
                { label: "Start Date *", key: "start", type: "date", placeholder: "" },
                { label: "End Date", key: "end", type: "date", placeholder: "" },
              ].map(({ label, key, placeholder, type }) => (
                <div key={key}>
                  <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{label}</label>
                  <input type={type ?? "text"} placeholder={placeholder} value={form[key as keyof typeof form]} onChange={(e) => setForm((p) => ({ ...p, [key]: e.target.value }))} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
                </div>
              ))}
              <div>
                <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Sector</label>
                <select value={form.sector} onChange={(e) => setForm((p) => ({ ...p, sector: e.target.value }))} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20">
      ...
      {viewEntry && (
        <Modal title="Project Details" onClose={() => setViewEntry(null)}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Project", value: viewEntry.project },
            ...
            <button onClick={() => setViewEntry(null)} className="w-full rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted">Close</button>
          </div>
        </Modal>
      )}
    </div>
