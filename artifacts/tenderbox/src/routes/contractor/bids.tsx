...
import {
  Send, Eye, AlertTriangle, FileText, ArrowUpDown,
  Plus, ChevronDown, ChevronUp, Building2, DollarSign, Loader2, CheckCircle, Clock,
} from "lucide-react";
import { Modal } from "@/components/Modal";
import { useToast } from "@/contexts/ToastContext";
import { RoleGuard } from "@/components/RoleGuard";

export const Route = createFileRoute("/contractor/bids")({ component: GuardedBidsPage });
...
interface Bid {
  id: string; trackingId: string; tenderRef: string; tenderTitle: string; entity: string;
  submissionDate: string; closingDate: string; bidAmount: number;
  ...
  rank: number | null; redFlags: string[];
}
...
function BidsPage() {
  ...
  const handleSubmitBid = () => {
    if (!bidAmount || parseFloat(bidAmount) <= 0) { toast("Please enter a valid bid amount", "error"); return; }
    if (Object.values(checkedDocs).filter(Boolean).length < 4) { toast("Please confirm at least 4 supporting documents are attached", "warning"); return; }
    setSubmitting(true);
    setTimeout(() => {
      ...
      const newBid: Bid = {
        id: String(Date.now()), trackingId, tenderRef: tender.ref, tenderTitle: tender.title, entity: tender.entity,
        ...
        status: "under_review", scores: { compliance: 88, functionality: null, price: null, bbbee: 95, total: null }, rank: null, redFlags: [],
      };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button onClick={() => setShowSubmitModal(true)} className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
          <Plus className="h-4 w-4" />Submit New Bid
        </button>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-6 py-3 font-medium cursor-pointer" onClick={() => toggleSort("submissionDate")}><span className="inline-flex items-center gap-1">Submitted <ArrowUpDown className="h-3 w-3" /></span></th>
                <th className="px-6 py-3 font-medium cursor-pointer" onClick={() => toggleSort("bidAmount")}><span className="inline-flex items-center gap-1">Amount <ArrowUpDown className="h-3 w-3" /></span></th>
                <th className="px-6 py-3 font-medium cursor-pointer" onClick={() => toggleSort("totalScore")}><span className="inline-flex items-center gap-1">Score <ArrowUpDown className="h-3 w-3" /></span></th>
                    ...
                    <tr key={bid.id} className={`${i % 2 === 1 ? "bg-muted/30" : "bg-card"} hover:bg-primary/5 transition-colors`}>
                      ...
                      <td className="px-6 py-4">
                        {bid.scores.total !== null
                          ? <span className={`font-medium ${bid.scores.total >= 80 ? "text-success" : bid.scores.total >= 60 ? "text-warning" : "text-danger"}`}>{bid.scores.total.toFixed(1)}%</span>
                          : <span className="text-muted-foreground text-xs">Pending</span>}
                      </td>
                      <td className="px-6 py-4">
                        {bid.rank !== null
                          ? <span className={`inline-flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ${bid.rank === 1 ? "bg-success text-success-foreground" : "bg-muted text-foreground"}`}>#{bid.rank}</span>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {bid.status === "draft" && (
                            <button onClick={() => { setBids((prev) => prev.map((b) => b.id === bid.id ? { ...b, status: "under_review", submissionDate: new Date().toISOString() } : b)); toast(`Bid ${bid.trackingId} submitted`, "success"); }} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90">
                        ...
                        <td colSpan={8} className="px-6 py-4">
                          ...
                          {bid.redFlags.length > 0 && (
                            <div className="mt-3 flex items-start gap-2 rounded-md border border-danger/20 bg-danger/5 p-3">
                              ...
                              <div>
                                ...
                                {bid.redFlags.map((f, fi) => <p key={fi} className="text-sm text-muted-foreground">{f}</p>)}
                              </div>
                            </div>
                          )}
                          <div className="mt-3 flex gap-2">
                            <button onClick={() => toast("Downloading bid document…", "info")} className="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted">
                              <FileText className="h-3.5 w-3.5" />Download Bid
                            </button>
                            <Link to="/contractor/tenders" className="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted">
                              View Tender
                            </Link>
                          </div>
                        </td>
          ...
          <div className="space-y-5">
            <div>
              ...
              <select value={selectedTender} onChange={(e) => setSelectedTender(e.target.value)} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20">
                {TENDERS_FOR_BID.map((t) => <option key={t.ref} value={t.ref}>{t.ref} — {t.title}</option>)}
              </select>
              {(() => {
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input type="number" placeholder="e.g. 12500000" value={bidAmount} onChange={(e) => setBidAmount(e.target.value)} className="w-full rounded-md border border-border bg-background pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              ...
              <div className="mt-2 space-y-2">
                {DOCS_CHECKLIST.map((doc) => (
                  <label key={doc} className="flex cursor-pointer items-center gap-3 rounded-md border border-border bg-muted/30 px-3 py-2 hover:bg-muted/60">
                    <input type="checkbox" checked={checkedDocs[doc] ?? false} onChange={(e) => setCheckedDocs((prev) => ({ ...prev, [doc]: e.target.checked }))} className="h-4 w-4 rounded border-border text-primary" />
                    <span className="text-sm text-foreground">{doc}</span>
                    {checkedDocs[doc] && <CheckCircle className="ml-auto h-4 w-4 text-success" />}
                  </label>
                ))}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{Object.values(checkedDocs).filter(Boolean).length} of {DOCS_CHECKLIST.length} confirmed</p>
            </div>
            <div className="rounded-md border border-warning/20 bg-warning/5 p-3 text-xs text-muted-foreground">
              <strong className="text-foreground">Declaration:</strong> By submitting this bid, your company confirms that all information is accurate and that you are compliant with all MFMA procurement requirements.
            </div>
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => setShowSubmitModal(false)} disabled={submitting} className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted disabled:opacity-60">Cancel</button>
            </div>
          </div>
        </Modal>
      )}
