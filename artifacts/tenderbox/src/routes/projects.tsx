...
import {
  ...
  MapPin, Building2, CheckCircle, Clock, AlertTriangle, X,
} from "lucide-react";
...
const INITIAL_PROJECTS: Project[] = [
  ...
  { id: "2", name: "Rehabilitation of Stormwater Drainage — Amathole", client: "Amathole District Municipality", location: "Amathole, Eastern Cape", province: "Eastern Cape", sector: "Stormwater", role: "Main Contractor", startDate: "2024-06-01", endDate: null, status: "Active", contractValue: 6890000, certifiedValue: 5245000, completion: 76, verificationStatus: "Verified", description: "Stormwater drainage rehabilitation in select urban areas including 4km of box culverts and 12 inlet structures.", siteManager: "Ms. T. Nkosi" },
  { id: "3", name: "Installation of Water Reticulation — KwaMashu", client: "eThekwini Water Services", location: "KwaMashu, KwaZulu-Natal", province: "KwaZulu-Natal", sector: "Water & Sanitation", role: "Main Contractor", startDate: "2023-09-15", endDate: "2025-01-30", status: "Completed", contractValue: 13450000, certifiedValue: 13450000, completion: 100, verificationStatus: "Verified", description: "Phase 2 water reticulation upgrade covering 5 suburbs. 8km of HDPE pipework, 240 service connections, 3 pump stations.", siteManager: "Eng. P. Dlamini" },
  { id: "4", name: "Upgrading of Access Roads — Matatiele", client: "Matatiele Local Municipality", location: "Matatiele, Eastern Cape", province: "Eastern Cape", sector: "Roads", role: "Main Contractor", startDate: "2026-03-01", endDate: null, status: "Active", contractValue: 8450000, certifiedValue: 1200000, completion: 14, verificationStatus: "Pending", description: "Phase 2 of rural access road upgrade programme. 12km of gravel road regrading and gravelling across 4 villages.", siteManager: "Mr. B. Cele" },
  { id: "5", name: "Solar PV Installation — Tshwane", client: "City of Tshwane", location: "Pretoria, Gauteng", province: "Gauteng", sector: "Renewable Energy", role: "JV Partner", startDate: "2025-07-01", endDate: "2026-01-31", status: "On Hold", contractValue: 22000000, certifiedValue: 8500000, completion: 38, verificationStatus: "Pending", description: "Installation of 2.4MWp rooftop solar PV across 18 municipal buildings. Currently on hold pending land rights resolution.", siteManager: "Eng. R. Okafor" },
];
...
function ProjectsPage() {
  ...
  const [form, setForm] = useState({
    name: "", client: "", location: "", province: PROVINCES[0], sector: SECTORS[0],
    role: "Main Contractor" as Project["role"], startDate: "", contractValue: "", description: "", siteManager: "",
  });
  ...
  const handleAddProject = () => {
    if (!form.name || !form.client || !form.startDate || !form.contractValue) {
    ...
    const newProject: Project = {
      ...
      endDate: null,
      status: "Active",
      contractValue: parseFloat(form.contractValue.replace(/[^0-9.]/g, "")),
      certifiedValue: 0,
      completion: 0,
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="text-right">
                    ...
                    <div className={`text-xs font-medium ${verCls}`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => toast(`Downloading completion certificate for ${project.name}`, "info")} className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "Role", value: viewProject.role },
                ...
                { label: "Site Manager", value: viewProject.siteManager ?? "—" },
                { label: "Verification", value: viewProject.verificationStatus },
            ...
            {viewProject.description && (
              <div className="rounded-md border border-border bg-muted/30 p-3">
                <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Project Description</div>
                <p className="mt-1 text-sm text-muted-foreground">{viewProject.description}</p>
              </div>
            )}
            <div className="flex gap-2 pt-1">
              <button onClick={() => { toast(`Downloading project report for ${viewProject.name}`, "info"); setViewProject(null); }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
                <Download className="h-4 w-4" />Download Project Report
              </button>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <select value={form.province} onChange={(e) => setForm((prev) => ({ ...prev, province: e.target.value }))} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20">
              ...
              <div>
                ...
                <select value={form.role} onChange={(e) => setForm((prev) => ({ ...prev, role: e.target.value as Project["role"] }))} className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20">
